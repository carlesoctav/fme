...
os.environ.setdefault(
    ...
    "--xla_dump_hlo_module_re=jit__step "
...
from src.training_utils import make_train_step, Optimizer
from src.utils import print_memory
from src.distributed import unbox_params
from src.benchmarking_utils import benchmark_loop
...
def loss_function(model, optimizer, batch, *, rngs):
    batch_size = batch.input_ids.shape[0]
...
def main():
    ...
    param_key = jr.PRNGKey(SEED)
    dropout_key = jr.PRNGKey(SEED)

    rngs = Rngs(params = param_key, dropout = dropout_key)
    rngs, model_rngs = split_rngs(rngs, 2)
    ...
    print(f"make_module_opt took {diff:.4f}s")
    ...
    print(f"DEBUGPRINT[44]: train.py:198: diff={diff}")
    benchmark_loop(model, optimizer, compile, train_loader, logger, rngs = rngs)
