...
os.environ.setdefault(
    ...
    "--xla_dump_hlo_pass_re=spmd_partitioning "
    "--xla_dump_hlo_module_re=jit_train_step "
    "--xla_dump_to=/tmp/xla_spmd",
)
os.environ.setdefault("TF_CPP_VMODULE", "xla_tpu_spmd_partitioning=1")
...
from src.logger import TrackioLogger, setup_logger
from src.data import make_dataloader
from src.data.masked_language_modeling import masked_language_modeling_transforms
    ...
    try:
        while step_idx < num_steps:
            ...
            with jax.profiler.StepTraceAnnotation("train_step", step_num=step_idx):
                ...
                jtu.tree_map(lambda x: x.block_until_ready() if hasattr(x, 'block_until_ready') else x, train_state)
                jtu.tree_map(lambda x: x.block_until_ready() if hasattr(x, 'block_until_ready') else x, optimizer_placeholder)
        ...
        if jax.process_index() == 0:
            LOGGER.info("=" * 50)
            LOGGER.info(" Flax BERT Benchmark Results (WITH DROPOUT) ".center(50, "="))
            LOGGER.info("=" * 50)
            LOGGER.info(f"Train Step Time (avg): {train_step_times.mean() * 1000:.1f}ms")
            LOGGER.info(f"Train Step Time (median): {np.median(train_step_times) * 1000:.1f}ms")
            LOGGER.info(f"Train Step Time (std): {train_step_times.std() * 1000:.1f}ms")
            LOGGER.info(f"Next Batch Time (avg): {next_batch_times.mean() * 1000:.1f}ms")
            LOGGER.info(f"Batches/sec (avg): {1.0 / train_step_times.mean():.2f}")
            if compile_step_time is not None:
                LOGGER.info(f"Compile Time: {compile_step_time:.2f}s")
            LOGGER.info("=" * 50)
...
def main():
    LOGGER.info("Starting Flax BERT Benchmark WITH DROPOUT")
    logger = TrackioLogger(project="benchmark", name="bert-flax-with-dropout")
    ...
    encoder_params = encoder.init(
        {'params': model_key, 'dropout': model_key},
        **dummy_input,
        enable_dropout=False,
    )
    ...
    mlm_params = mlm_head.init(
        {'params': mlm_key, 'dropout': mlm_key},
        dummy_encoded,
        masked_positions=None,
    )
    ...
    lower = train_step_fn.lower(train_state, optimizer_placeholder, batch, key)

    start = time.monotonic()
    compile = lower.compile()
    hlo_txt = lower.as_text()
    with open("./benchmark/flax_hlo_no_dropout.txt", "w") as f:
        f.write(hlo_txt)

    diff = time.monotonic() - start
    print(f"DEBUGPRINT[45]: bert_flax_train.py:398: diff={diff}")
...
if __name__ == "__main__":
    setup_logger(log_file="./research/bert-mlm/flax_dropout_run.log")
    main()
