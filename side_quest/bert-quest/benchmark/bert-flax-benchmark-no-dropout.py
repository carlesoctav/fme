...
def benchmark_loop_flax(
    ...
    try:
        ...
        if jax.process_index() == 0:
            LOGGER.info("=" * 50)
            LOGGER.info(" Flax BERT Benchmark Results (NO DROPOUT) ".center(50, "="))
            LOGGER.info("=" * 50)
...
def main():
    LOGGER.info("Starting Flax BERT Benchmark WITHOUT DROPOUT")
    logger = TrackioLogger(project="benchmark", name="bert-flax-no-dropout")
    ...
    train_state, optimizer_placeholder, stats = benchmark_loop_flax(
        ...
        num_steps=NUM_STEPS,
        trace_steps=(1, 10),
        trace_dir="./trace_flax_no_dropout",
        key=key,
    )
...
if __name__ == "__main__":
    setup_logger(log_file="./research/bert-mlm/flax_no_dropout_run.log")
    main()
