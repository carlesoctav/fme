...
from src.data.training import make_dataloader
...
def test_filter_jit(model, optimizer, train_loader, key):
    ...
    print(f"Function creation time: {compile_end - compile_start:.3f}s")
    ...
    for step in range(NUM_STEPS):
        ...
        with jax.profiler.StepTraceAnnotation("train_step", step_num=step):
            current_model, current_opt, aux = train_step(
                current_model, current_opt, batch, key=step_key
            )
            jtu.tree_map(
        ...
        if step > 0:
            ...
            print(f"First step (compilation + execution) time: {end - start:.3f}s")
...
def test_manual_jit(model, optimizer, train_loader, key):
    ...
    for step in range(NUM_STEPS):
        ...
        start = time.monotonic()
        with jax.profiler.StepTraceAnnotation("train_step", step_num=step):
            current_params, opt_state, aux = train_step(
                current_params, opt_state, batch, step_key
            )
            jtu.tree_map(
                lambda x: x.block_until_ready()
                if hasattr(x, "block_until_ready")
