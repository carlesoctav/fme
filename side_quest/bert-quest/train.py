...
from src.training_utils import make_module_opt, make_train_step, benchmark_loop
from src.metrics import SufficientMetric
...
def main():
    ...
    compiled_text = train_step_fn.lower(
        model, optimizer, batch, key=jax.random.key(10)
    ).as_text()
    compiled = train_step_fn.lower(
        model, optimizer, batch, key=jax.random.key(10)
    ).compile()
    print(f"compiled: {compiled}")
    diff = time.monotonic() - timing
    print_memory(compiled.compiled.memory_analysis())
    print(f"compile took {diff:.4f}s")
