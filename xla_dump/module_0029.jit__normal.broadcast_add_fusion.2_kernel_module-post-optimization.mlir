module @broadcast_add_fusion.2_kernel_module attributes {dlti.dl_spec = #dlti.dl_spec<index = 64 : i32>, xla.cpu_memory_region_name = "xla_cpu_emitter__loop_fusion_kernel_emitter__hlo_opcode__fusion"} {
  func.func @broadcast_add_fusion.2(%arg0: tensor<2xi32> {llvm.align = 64 : index, llvm.dereferenceable = 8 : index, xla.invariant, xla.slice_index = 0 : index}, %arg1: tensor<96xi32> {llvm.align = 64 : index, llvm.dereferenceable = 384 : index, xla.slice_index = 1 : index}) -> tensor<96xi32> attributes {xla.backend_kind = #xla.backend_kind<cpu>, xla.entry} {
    ...
    %0 = scf.for %arg2 = %c0 to %c12 step %c1 iter_args(%arg3 = %arg1) -> (tensor<96xi32>) {
      ...
      %3 = scf.for %arg4 = %c0 to %c8 step %c1 iter_args(%arg5 = %arg3) -> (tensor<96xi32>) {
        ...
        %8 = xla.apply_indexing #xla.indexing_map<"(d0, d1) -> (d0 * 8 + d1), domain: d0 in [0, 11], d1 in [0, 7]">(%arg2, %arg4)
        %inserted = tensor.insert %7 into %arg5[%8] {alias_scopes = [#llvm.alias_scope<id = distinct[0]<>, domain = <id = distinct[1]<>>, description = "xla.slice_index=1">]} : tensor<96xi32>
        scf.yield %inserted : tensor<96xi32>
      }
      scf.yield %3 : tensor<96xi32>
    }
    return %0 : tensor<96xi32>
  }
}