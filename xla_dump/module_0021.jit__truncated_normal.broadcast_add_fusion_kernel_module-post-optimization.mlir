module @broadcast_add_fusion_kernel_module attributes {dlti.dl_spec = #dlti.dl_spec<index = 64 : i32>, xla.cpu_memory_region_name = "xla_cpu_emitter__loop_fusion_kernel_emitter__hlo_opcode__fusion"} {
  func.func @broadcast_add_fusion(%arg0: tensor<i32> {llvm.align = 64 : index, llvm.dereferenceable = 4 : index, xla.invariant, xla.slice_index = 0 : index}, %arg1: tensor<4xi32> {llvm.align = 64 : index, llvm.dereferenceable = 16 : index, xla.invariant, xla.slice_index = 1 : index}, %arg2: tensor<512xi32> {llvm.align = 64 : index, llvm.dereferenceable = 2048 : index, xla.invariant, xla.slice_index = 2 : index}, %arg3: tensor<512xi32> {llvm.align = 64 : index, llvm.dereferenceable = 2048 : index, xla.invariant, xla.slice_index = 3 : index}, %arg4: tensor<i32> {llvm.align = 64 : index, llvm.dereferenceable = 4 : index, xla.invariant, xla.slice_index = 4 : index}, %arg5: tensor<512xi32> {llvm.align = 64 : index, llvm.dereferenceable = 2048 : index, xla.slice_index = 5 : index}) -> tensor<512xi32> attributes {xla.backend_kind = #xla.backend_kind<cpu>, xla.entry} {
    ...
    %13 = scf.for %arg6 = %c0 to %c32 step %c1 iter_args(%arg7 = %arg5) -> (tensor<512xi32>) {
      %14 = scf.for %arg8 = %c0 to %c16 step %c1 iter_args(%arg9 = %arg7) -> (tensor<512xi32>) {
        %15 = xla.apply_indexing #xla.indexing_map<"(d0, d1) -> (d0 * 16 + d1), domain: d0 in [0, 31], d1 in [0, 15]">(%arg6, %arg8)
        %extracted_5 = tensor.extract %arg2[%15] {llvm.noalias = [#llvm.alias_scope<id = distinct[0]<>, domain = <id = distinct[1]<>>, description = "xla.slice_index=5">]} : tensor<512xi32>
        ...
        %extracted_6 = tensor.extract %arg3[%15] {llvm.noalias = [#llvm.alias_scope<id = distinct[0]<>, domain = <id = distinct[1]<>>, description = "xla.slice_index=5">]} : tensor<512xi32>
        ...
        %inserted = tensor.insert %45 into %arg9[%15] {alias_scopes = [#llvm.alias_scope<id = distinct[0]<>, domain = <id = distinct[1]<>>, description = "xla.slice_index=5">]} : tensor<512xi32>
        scf.yield %inserted : tensor<512xi32>
      }
      scf.yield %14 : tensor<512xi32>
    }
    return %13 : tensor<512xi32>
  }
}