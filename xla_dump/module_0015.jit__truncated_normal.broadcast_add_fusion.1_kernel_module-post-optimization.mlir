module @broadcast_add_fusion.1_kernel_module attributes {dlti.dl_spec = #dlti.dl_spec<index = 64 : i32>, xla.cpu_memory_region_name = "xla_cpu_emitter__loop_fusion_kernel_emitter__hlo_opcode__fusion"} {
  func.func @broadcast_add_fusion.1(%arg0: tensor<i32> {llvm.align = 64 : index, llvm.dereferenceable = 4 : index, xla.invariant, xla.slice_index = 0 : index}, %arg1: tensor<4xi32> {llvm.align = 64 : index, llvm.dereferenceable = 16 : index, xla.invariant, xla.slice_index = 1 : index}, %arg2: tensor<128xi32> {llvm.align = 64 : index, llvm.dereferenceable = 512 : index, xla.invariant, xla.slice_index = 2 : index}, %arg3: tensor<128xi32> {llvm.align = 64 : index, llvm.dereferenceable = 512 : index, xla.invariant, xla.slice_index = 3 : index}, %arg4: tensor<128xi32> {llvm.align = 64 : index, llvm.dereferenceable = 512 : index, xla.slice_index = 4 : index}) -> tensor<128xi32> attributes {xla.backend_kind = #xla.backend_kind<cpu>, xla.entry} {
    ...
    %9 = scf.for %arg5 = %c0 to %c16 step %c1 iter_args(%arg6 = %arg4) -> (tensor<128xi32>) {
      %10 = scf.for %arg7 = %c0 to %c8 step %c1 iter_args(%arg8 = %arg6) -> (tensor<128xi32>) {
        %11 = xla.apply_indexing #xla.indexing_map<"(d0, d1) -> (d0 * 8 + d1), domain: d0 in [0, 15], d1 in [0, 7]">(%arg5, %arg7)
        %extracted_3 = tensor.extract %arg2[%11] {llvm.noalias = [#llvm.alias_scope<id = distinct[0]<>, domain = <id = distinct[1]<>>, description = "xla.slice_index=4">]} : tensor<128xi32>
        ...
        %extracted_4 = tensor.extract %arg3[%11] {llvm.noalias = [#llvm.alias_scope<id = distinct[0]<>, domain = <id = distinct[1]<>>, description = "xla.slice_index=4">]} : tensor<128xi32>
        ...
        %inserted = tensor.insert %34 into %arg8[%11] {alias_scopes = [#llvm.alias_scope<id = distinct[0]<>, domain = <id = distinct[1]<>>, description = "xla.slice_index=4">]} : tensor<128xi32>
        scf.yield %inserted : tensor<128xi32>
      }
      scf.yield %10 : tensor<128xi32>
    }
    return %9 : tensor<128xi32>
  }
}