module @maximum_minimum_fusion_kernel_module attributes {dlti.dl_spec = #dlti.dl_spec<index = 64 : i32>, xla.cpu_memory_region_name = "xla_cpu_emitter__loop_fusion_kernel_emitter__hlo_opcode__fusion"} {
  func.func @maximum_minimum_fusion(%arg0: tensor<f32> {llvm.align = 64 : index, llvm.dereferenceable = 4 : index, xla.invariant, xla.slice_index = 0 : index}, %arg1: tensor<f32> {llvm.align = 64 : index, llvm.dereferenceable = 4 : index, xla.invariant, xla.slice_index = 1 : index}, %arg2: tensor<128xi32> {llvm.align = 64 : index, llvm.dereferenceable = 512 : index, xla.invariant, xla.slice_index = 2 : index}, %arg3: tensor<128xi32> {llvm.align = 64 : index, llvm.dereferenceable = 512 : index, xla.invariant, xla.slice_index = 3 : index}, %arg4: tensor<i32> {llvm.align = 64 : index, llvm.dereferenceable = 4 : index, xla.invariant, xla.slice_index = 4 : index}, %arg5: tensor<i32> {llvm.align = 64 : index, llvm.dereferenceable = 4 : index, xla.invariant, xla.slice_index = 5 : index}, %arg6: tensor<128xf32> {llvm.align = 64 : index, llvm.dereferenceable = 512 : index, xla.slice_index = 6 : index}) -> tensor<128xf32> attributes {xla.backend_kind = #xla.backend_kind<cpu>, xla.entry} {
    ...
    %29 = scf.for %arg7 = %c0 to %c16 step %c1 iter_args(%arg8 = %arg6) -> (tensor<128xf32>) {
      %30 = scf.for %arg9 = %c0 to %c8 step %c1 iter_args(%arg10 = %arg8) -> (tensor<128xf32>) {
        %31 = xla.apply_indexing #xla.indexing_map<"(d0, d1) -> (d0 * 8 + d1), domain: d0 in [0, 15], d1 in [0, 7]">(%arg7, %arg9)
        %extracted_27 = tensor.extract %arg2[%31] {llvm.noalias = [#llvm.alias_scope<id = distinct[0]<>, domain = <id = distinct[1]<>>, description = "xla.slice_index=6">]} : tensor<128xi32>
        %extracted_28 = tensor.extract %arg3[%31] {llvm.noalias = [#llvm.alias_scope<id = distinct[0]<>, domain = <id = distinct[1]<>>, description = "xla.slice_index=6">]} : tensor<128xi32>
        ...
        %inserted = tensor.insert %81 into %arg10[%31] {alias_scopes = [#llvm.alias_scope<id = distinct[0]<>, domain = <id = distinct[1]<>>, description = "xla.slice_index=6">]} : tensor<128xf32>
        scf.yield %inserted : tensor<128xf32>
      }
      scf.yield %30 : tensor<128xf32>
    }
    return %29 : tensor<128xf32>
  }
}